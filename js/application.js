  ...
  $(function () {
    ...
    setTimeout(function () {
      ...
      $sponsors.affix({
        offset: {
          top: function () {
            var offsetTop      = $sponsors.offset().top
            var sponsorsMargin  = parseInt($sponsors.children(0).css('margin-top'), 10)
            var navOuterHeight = $('.docs-nav').height()

            return (this.top = offsetTop - navOuterHeight - sponsorsMargin)
          },
          bottom: function () {
            return (this.bottom = $('.footer').outerHeight(true))
          }
